...
event_response_t msr_syscall_sysenter_cb(vmi_instance_t vmi, vmi_event_t *event)
{
    ...
    if((unsigned int)rax<2000&&(unsigned int)rax>=0)
    {
        printf("\nmsr Syscall happened: RAX(syscall#)=%u RDI(1st argument)=%u ", (unsigned int)rax, (unsigned int)rdi);
    }
...
event_response_t syscall_compat_cb(vmi_instance_t vmi, vmi_event_t *event)
{
    ...
    printf("Syscall compat happened: RAX(syscall#)=%u RDI(1st argument)=%u\n", (unsigned int)rax, (unsigned int)rdi);
...
event_response_t vsyscall_cb(vmi_instance_t vmi, vmi_event_t *event)
{
    ...
    printf("vSyscall happened: RAX(syscall#)=%u RDI(1st argument)=%u\n", (unsigned int)rax, (unsigned int)rdi);
...
event_response_t ia32_sysenter_target_cb(vmi_instance_t vmi, vmi_event_t *event)
{
    ...
    printf("is32 Syscall happened: RAX(syscall#)=%u RDI(1st argument)=%u\n", (unsigned int)rax, (unsigned int)rdi);
...
event_response_t syscall_lm_cb(vmi_instance_t vmi, vmi_event_t *event)
{
    ...
    printf("Syscall lm happened: RAX(syscall#)=%u RDI(1st argument)=%u\n", (unsigned int)rax, (unsigned int)rdi);
...
event_response_t cr3_one_task_callback(vmi_instance_t vmi, vmi_event_t *event)
{
    ...
    if(event->reg_event.value == cr3)
    {
        ...
        //fprintf(stderr, "msr Could not install sysenter syscall handler.\n");
        //if(vmi_register_event(vmi, &kernel_sysenter_target_event) == VMI_FAILURE)
        //   fprintf(stderr, "Could not install sysenter  handler.\n");
        //if(vmi_register_event(vmi, &kernel_vsyscall_event) == VMI_FAILURE)
        //  fprintf(stderr, "Could not install  syscall handler.\n");
    }
    else
    {
...
event_response_t cr3_all_tasks_callback(vmi_instance_t vmi, vmi_event_t *event)
{
   ...
   //printf(" PID %i ",pid);
    //event.reg_event.equal = vmi_pid_to_dtb(vmi, pid);
    msr_syscall_sysenter_event.mem_event.in_access = VMI_MEMACCESS_X;
    msr_syscall_sysenter_event.callback=msr_syscall_sysenter_cb;
    msr_syscall_compat_event.mem_event.in_access = VMI_MEMACCESS_X;
    msr_syscall_compat_event.callback = syscall_compat_cb;
    kernel_sysenter_target_event.mem_event.in_access = VMI_MEMACCESS_X;
    kernel_sysenter_target_event.callback=ia32_sysenter_target_cb;
    kernel_vsyscall_event.mem_event.in_access = VMI_MEMACCESS_X;
    kernel_vsyscall_event.callback=vsyscall_cb;

    if(vmi_register_event(vmi, &msr_syscall_sysenter_event) == VMI_SUCCESS){
        printf(" PID %i ",pid);
    }
    if(vmi_register_event(vmi, &msr_syscall_compat_event) == VMI_SUCCESS){
        printf(" PID %i ",pid);
    }
    vmi_register_event(vmi, &kernel_sysenter_target_event);
    vmi_register_event(vmi, &kernel_vsyscall_event);
...
int main (int argc, char **argv)
{
    ...
    memset(&msr_syscall_compat_event, 0, sizeof(vmi_event_t));
    msr_syscall_compat_event.version = VMI_EVENTS_VERSION;
    msr_syscall_compat_event.type = VMI_EVENT_MEMORY;
    msr_syscall_compat_event.mem_event.gfn = phys_cstar >> 12;
